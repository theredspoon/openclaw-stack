...
function mediaDirectoryPage(dirPath, urlPath) {
  ...
  const rows = [
    ...dirs.map(
      (e) =>
        `<tr><td>&#128193; <a href="${prefix}${e.name}/">${e.name}/</a></td><td>&mdash;</td><td></td></tr>`
    ),
    ...files.map(
      (e) =>
        `<tr><td>&#128196; <a href="${prefix}${e.name}">${e.name}</a></td><td>${formatSize(
          e.size
        )}</td><td><a href="${prefix}${e.name}" download class="dl">&#8681;</a></td></tr>`
    ),
  ]
...
const server = createServer(async (req, res) => {
  ...
  if (!BP) {
    ...
    if (
      ...
      if (!effectiveBP) {
        ...
        console.log(
          `[dashboard] Auto-detected base path: ${effectiveBP} (set DASHBOARD_BASE_PATH=${effectiveBP} to make this explicit)`
        )
      }
  ...
  if (subPath === '/') {
    ...
    const wsPrefix = effectiveBP.startsWith('/') ? effectiveBP.slice(1) : effectiveBP
    res.writeHead(302, {
      Location: `${effectiveBP}/browser/${agentId}/vnc.html?path=${
        wsPrefix ? wsPrefix + '/' : ''
...
server.on('upgrade', async (req, socket, head) => {
  ...
  if (PAIRING_AUTH_ENABLED) {
    ...
    if (!session) {
      console.log(
        `[dashboard] WS pairing auth denied: no valid session cookie (${req.socket.remoteAddress})`
      )
      socket.destroy()
      return
    }
