...
#   INSTANCE_NAME=<name>: Deploy config for a single claw only (openclaw.json, agent dirs)
#
# Interface:
...
#                LOG_WORKER_TOKEN, LOG_WORKER_URL,
...
" "${config_target}/openclaw.json"
  ...
  # Pre-create per-agent session stores — the gateway lazily creates these dirs
  # only when the first session is saved, but `openclaw doctor` reports CRITICAL if missing.
  # models.json is NOT deployed here — it's generated by OpenClaw at runtime from
  # the models.providers section in openclaw.json (resolved via entrypoint envsubst).
  ...
  echo "  Pre-created agent directories" >&2
...
# ── Per-claw config (openclaw.json, agent dirs) ──────────────────────
