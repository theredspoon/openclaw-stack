...
Passwords are auto-generated by `npm run pre-deploy` and stored in `.env.local`. Source `scripts/lib/source-config.sh` locally before running the SSH block to get `$ADMINCLAW_PASSWORD` and `$OPENCLAW_PASSWORD`.
...
> **Note:** Passwords are persisted in `.env.local` and always available via `source scripts/lib/source-config.sh`. No need to save them from this output — they are retrieved automatically for the deployment report (`08c-deploy-report.md`).
