...
Set the admin token (auto-generated by `npm run pre-deploy` into `.env.local`) to protect `/admin/*` management endpoints:
...
> The token is persisted in `.env.local` and always available via `source-config.sh` — no need to save it manually.
