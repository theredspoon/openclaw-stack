...
|--------|------------|------|-------|
| (generated by `openclaw-multi.sh`) | `/home/openclaw/openclaw/docker-compose.override.yml` | generated | Per-claw service definitions |
...
>
> `for CLAW in $CLAWS; do sudo docker logs "$CLAW"; done`
>
> Common issues:
>
...
(before gosu drops to node). Fix ownership so each claw can write to them:
...
> **Note:** Sandbox images are per-claw (each has its own nested Docker daemon). This test verifies the first claw; all claws build identical images from the same toolkit config.
...
timeout 600 bash -c "until sudo docker logs $FIRST_CLAW 2>&1 | grep -q 'Executing as node'; do sleep 5; done"
...
for img in openclaw-sandbox:bookworm-slim openclaw-sandbox-toolkit:bookworm-slim \
  ...
  if sudo docker exec "$FIRST_CLAW" docker image inspect "$img" > /dev/null 2>&1; then
    echo "  $img: EXISTS"
  else
    echo "  $img: MISSING"
    FAILED=1
...
for img in openclaw-sandbox-toolkit:bookworm-slim; do
  USER=$(sudo docker exec "$FIRST_CLAW" docker image inspect "$img" 2>/dev/null \
...
for bin in go rustc bun brew node npm pnpm git curl wget jq ffmpeg convert claude gifgrep; do
  if sudo docker exec "$FIRST_CLAW" docker run --rm openclaw-sandbox-toolkit:bookworm-slim which "$bin" > /dev/null 2>&1; then
    echo "  toolkit/$bin: OK"
  else
...
if sudo docker exec "$FIRST_CLAW" docker images | grep -q base-root; then
...
for img in openclaw-sandbox-toolkit:bookworm-slim openclaw-sandbox-browser:bookworm-slim; do
  BUILD_DATE=$(sudo docker exec "$FIRST_CLAW" docker image inspect "$img" \
...
if [ "$FAILED" -eq 1 ]; then
  ...
  echo "  sudo docker logs $FIRST_CLAW 2>&1 | grep '\\[entrypoint\\]'"
fi
```
