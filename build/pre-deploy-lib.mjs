...
/**
 * Compute derived values per claw for the Handlebars template.
 * Pure data transformation — no side effects.
 * @param {Record<string, object>} claws - Merged claw configs (mutated in place)
 * @param {object} stack - Stack config (mutated: sets stack.vector, egress/registry tokens)
 * @param {object} host - Host config
 * @param {object|null} previousDeploy - Previous deploy config for token caching
 * @param {(value: string, cachePath: string, prev: object|null) => string} resolveTokenFn
 * @returns {Record<string, string>} autoTokens — env var name → value for .env persistence
 */
export function computeDerivedValues(claws, stack, host, previousDeploy, resolveTokenFn) {
  ...
  if (stack.egress_proxy) {
    ...
    stack.egress_proxy.auth_token = resolveTokenFn(
      before,
  ...
  if (stack.sandbox_registry) {
    ...
    sr.token = resolveTokenFn(before, "stack.sandbox_registry.token", previousDeploy);
